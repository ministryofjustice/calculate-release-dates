...
export default class CalculateReleaseDatesApiClient extends RestClient {
  ...
  getPersonComparisonInputData(
    comparisonReference: string,
    mismatchReference: string,
    username: string,
  ): Promise<PersonComparisonInputs> {
    return this.get<PersonComparisonInputs>(
      {
        path: `/comparison/${comparisonReference}/mismatch/${mismatchReference}/input-data`,
      },
      asSystem(username),
    )
  }
  ...
  confirmSecondCheck(calculationRequestId: number, token: string): Promise<ConfirmSecondCheckResult> {
