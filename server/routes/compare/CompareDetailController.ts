...
export default class CompareDetailController implements Controller {
  ...
  POST = async (req: Request, res: Response): Promise<void> => {
    ...
    if (validationErrors.length === 0) {
      ...
      res.redirect(`/compare/result/${bulkComparisonResultId}`)
      return
    }
  ...
  private validateFormData(formData: {
    ...
    formCauses.forEach((cause: string) => {
      const subCategory = cause === 'OTHER' ? cause : (formData[`${cause}-subType`] as string | undefined)
      if (!subCategory) {
