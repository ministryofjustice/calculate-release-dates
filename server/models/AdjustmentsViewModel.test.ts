...
import { AdjustmentViewModel } from './ViewRouteAdjustmentsViewModel'
...
describe('AdjustmentsViewModel', () => {
  ...
  it.each([
    ...
    (adjustmentType: string, accessor: keyof AdjustmentsViewModel) => {
      const adjustments = {
        sentenceAdjustments: [
          {
            sentenceSequence: 1,
      ...
      expect((model[accessor] as AdjustmentViewModel).details).toHaveLength(1)
    },
  )
  it.each([
    ['RECALL_SENTENCE_REMAND', 'recallSentenceRemand'],
    ...
    'Should filter sentence adjustments by is active for %s',
    (adjustmentType: string, accessor: keyof AdjustmentsViewModel) => {
      const adjustments = {
        sentenceAdjustments: [
          {
      ...
      const model = new AdjustmentsViewModel(adjustments, sentencesAndOffences)
      const modelElement = model[accessor] as AdjustmentViewModel
      expect(modelElement.details).toHaveLength(1)
      expect(modelElement.details).toStrictEqual([
        {
          days: 999,
          from: '2021-02-01',
          sentence: 1,
  ...
  it.each([
    ...
    'Should sort sentence adjustments by from date for %s',
    (adjustmentType: string, accessor: keyof AdjustmentsViewModel) => {
      const adjustments = {
        sentenceAdjustments: [
          {
      ...
      const model = new AdjustmentsViewModel(adjustments, sentencesAndOffences)
      const modelElement = model[accessor] as AdjustmentViewModel
      expect(modelElement.details).toHaveLength(2)
      expect(modelElement.details).toStrictEqual([
        {
          days: 98765,
          from: '2021-02-01',
  ...
  it.each([
    ...
    'Should sort booking adjustments by from date for %s',
    (adjustmentType: string, accessor: keyof AdjustmentsViewModel) => {
      const adjustments = {
        sentenceAdjustments: [],
        bookingAdjustments: [
  ...
  it.each([
    ...
    'Should filter booking adjustments by is active %s',
    (adjustmentType: string, accessor: keyof AdjustmentsViewModel) => {
      const adjustments = {
        sentenceAdjustments: [],
        bookingAdjustments: [
